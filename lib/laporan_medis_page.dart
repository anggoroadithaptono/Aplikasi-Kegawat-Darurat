...
class LaporanMedisPage extends StatefulWidget {
  final String role; // Role ini bisa Anda set 'medis'

  const LaporanMedisPage({super.key, required this.role});

  @override
  State<LaporanMedisPage> createState() => _LaporanMedisPageState();
}

class _LaporanMedisPageState extends State<LaporanMedisPage> {
  final uraianController = TextEditingController();
  final jenisDaruratController = TextEditingController(); // Tambahan untuk Medis
  final jumlahKorbanController = TextEditingController(); // Tambahan untuk Medis
  final kondisiPasienController = TextEditingController(); // Tambahan untuk Medis
  final penangananAwalController = TextEditingController(); // Tambahan untuk Medis

  File? foto;
  File? video;
  final picker = ImagePicker();

  String? _selectedDaerah; // Ganti dengan pilihan daerah yang relevan untuk Medis

  final List<String> _daerahMedisList = [
    'RSUD Dr. Soetomo',
  ...
  final Map<String, String> _instansiMedisMapping = {
    'RSUD Dr. Soetomo': 'RSUD Dr. Soetomo',
    'Rumah Sakit Haji Surabaya': 'Rumah Sakit Haji Surabaya',
    'Puskesmas A': 'Puskesmas A',
    'Puskesmas B': 'Puskesmas B',
  };
  ...
  final Color primaryColor = const Color(0xFFC71811); // Warna merah dari HomeScreen
  ...
  void dispose() {
    ...
    jenisDaruratController.dispose();
    jumlahKorbanController.dispose();
    kondisiPasienController.dispose();
    penangananAwalController.dispose();
    super.dispose();
  }
  ...
  Future<void> ambilFoto() async {
    ...
    if (picked != null) {
      ...
      // Pastikan showBerhasilDialog mengembalikan Future<void> untuk digunakan dengan await
  ...
  void simpanLaporan() async {
    ...
    // Pastikan Anda sudah membuat MedisModel.dart
    final laporanMedisBaru = MedisModel(
      lokasi: _selectedDaerah!,
      uraian: uraianController.text,
      jenisDarurat: jenisDaruratController.text.isNotEmpty ? jenisDaruratController.text : null,
      jumlahKorban: int.tryParse(jumlahKorbanController.text) ?? 0,
      kondisiPasien: kondisiPasienController.text.isNotEmpty ? kondisiPasienController.text : null,
      penangananAwal: penangananAwalController.text.isNotEmpty ? penangananAwalController.text : null,
      waktu: waktu,
      jamMasuk: jamMasuk,
      status: 'Pending',
      fotoPath: foto?.path,
    ...
    try {
      ...
      await dbHelper.insertMedis(laporanMedisBaru.toMap()); // Gunakan toMap() dari MedisModel

      // Menampilkan dialog berhasil dan kemudian menutup halaman laporan
      await showBerhasilDialog(context, 'Laporan Medis berhasil terkirim'); // Menggunakan await karena showBerhasilDialog sekarang mengembalikan Future
      ...
      setState(() {
        ...
        jenisDaruratController.clear();
        jumlahKorbanController.clear();
        kondisiPasienController.clear();
        penangananAwalController.clear();
        foto = null;
        video = null;
        _videoController?.dispose();
        _videoController = null;
      ...
      print("Error menyimpan laporan Medis ke database: $e");
      if (mounted) { // Pastikan widget masih ada di tree sebelum menampilkan SnackBar
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Gagal mengirim laporan Medis: ${e.toString()}", style: GoogleFonts.poppins(color: Colors.white)),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                Expanded(
                  child: DropdownButtonFormField<String>(
                    ...
                    items: _daerahMedisList.map<DropdownMenuItem<String>>((String value) {
            ...
            // --- Field Spesifik Medis ---
            Text("Jenis Darurat Medis", style: GoogleFonts.poppins(color: primaryColor, fontWeight: FontWeight.bold, fontSize: 16)), // Menggunakan primaryColor
            const SizedBox(height: 8),
            TextFormField(
              controller: jenisDaruratController,
              decoration: InputDecoration( // Ubah ke InputDecoration karena focusedBorder bukan const
                hintText: 'Misal: Kecelakaan/Sakit Mendadak/Bencana',
                border: border,
                enabledBorder: border,
                focusedBorder: focusedBorder, // Menggunakan focusedBorder yang baru
                contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 14),
            ...
            Text("Jumlah Korban", style: GoogleFonts.poppins(color: primaryColor, fontWeight: FontWeight.bold, fontSize: 16)), // Menggunakan primaryColor
            const SizedBox(height: 8),
            TextFormField(
            ...
            Text("Kondisi Pasien", style: GoogleFonts.poppins(color: primaryColor, fontWeight: FontWeight.bold, fontSize: 16)), // Menggunakan primaryColor
            const SizedBox(height: 8),
            TextFormField(
              controller: kondisiPasienController,
              decoration: InputDecoration( // Ubah ke InputDecoration karena focusedBorder bukan const
                hintText: 'Kondisi pasien (stabil/kritis/dll.)',
                border: border,
                enabledBorder: border,
                focusedBorder: focusedBorder, // Menggunakan focusedBorder yang baru
                contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 14),
            ...
            Text("Penanganan Awal", style: GoogleFonts.poppins(color: primaryColor, fontWeight: FontWeight.bold, fontSize: 16)), // Menggunakan primaryColor
            const SizedBox(height: 8),
            TextFormField(
              controller: penangananAwalController,
              maxLines: 3,
              decoration: InputDecoration( // Ubah ke InputDecoration karena focusedBorder bukan const
                hintText: 'Penanganan awal yang telah dilakukan',
                border: border,
                enabledBorder: border,
                focusedBorder: focusedBorder, // Menggunakan focusedBorder yang baru
            ...
            TextFormField(
              ...
              maxLines: 4,
              decoration: InputDecoration( // Ubah ke InputDecoration karena focusedBorder bukan const
                hintText: 'Isi detail laporan medis',
                border: border,
                enabledBorder: border,
                focusedBorder: focusedBorder, // Menggunakan focusedBorder yang baru
                contentPadding: EdgeInsets.all(12),
