...
class LaporanPemadamPage extends StatefulWidget {
  final String role; // Role ini bisa Anda set 'pemadam'

  const LaporanPemadamPage({super.key, required this.role});

  @override
  State<LaporanPemadamPage> createState() => _LaporanPemadamPageState();
}

class _LaporanPemadamPageState extends State<LaporanPemadamPage> {
  final uraianController = TextEditingController();
  final jenisKebakaranController = TextEditingController(); // Tambahan untuk Pemadam
  final korbanJiwaController = TextEditingController(); // Tambahan untuk Pemadam
  final kerugianEstimasiController = TextEditingController(); // Tambahan untuk Pemadam

  File? foto;
  File? video;
  final picker = ImagePicker();

  String? _selectedDaerah; // Ganti dengan pilihan daerah yang relevan untuk Pemadam

  final List<String> _daerahPemadamList = [
    'Surabaya Utara',
    'Surabaya Selatan',
    'Surabaya Timur',
    'Surabaya Barat',
  ...
  // Anda mungkin perlu mapping instansi pemadam yang berbeda
  final Map<String, String> _instansiPemadamMapping = {
    'Surabaya Utara': 'Dinas Pemadam Kebakaran Surabaya Utara',
    'Surabaya Selatan': 'Dinas Pemadam Kebakaran Surabaya Selatan',
    'Surabaya Timur': 'Dinas Pemadam Kebakaran Surabaya Timur',
    'Surabaya Barat': 'Dinas Pemadam Kebakaran Surabaya Barat',
    'Surabaya Pusat': 'Dinas Pemadam Kebakaran Surabaya Pusat',
  };
  ...
  void dispose() {
    ...
    jenisKebakaranController.dispose();
    korbanJiwaController.dispose();
    kerugianEstimasiController.dispose();
    super.dispose();
  }
  ...
  void simpanLaporan() async {
    ...
    // Pastikan Anda sudah membuat PemadamModel.dart
    final laporanPemadamBaru = PemadamModel(
      lokasi: _selectedDaerah!,
      uraian: uraianController.text,
      jenisKebakaran: jenisKebakaranController.text.isNotEmpty ? jenisKebakaranController.text : null,
      korbanJiwa: int.tryParse(korbanJiwaController.text) ?? 0,
      kerugianEstimasi: kerugianEstimasiController.text.isNotEmpty ? kerugianEstimasiController.text : null,
      waktu: waktu,
      jamMasuk: jamMasuk,
      status: 'Pending',
      fotoPath: foto?.path,
    ...
    try {
      ...
      await dbHelper.insertPemadam(laporanPemadamBaru.toMap()); // Gunakan toMap() dari PemadamModel

      // Menampilkan dialog berhasil dan kemudian menutup halaman laporan
      await showBerhasilDialog(context, 'Laporan Pemadam berhasil terkirim'); // Menggunakan await karena showBerhasilDialog sekarang mengembalikan Future
      ...
      // Reset state setelah pop
      setState(() {
        _selectedDaerah = null;
        uraianController.clear();
        jenisKebakaranController.clear();
        korbanJiwaController.clear();
        kerugianEstimasiController.clear();
        foto = null;
        video = null;
        _videoController?.dispose();
        _videoController = null;
      ...
      print("Error menyimpan laporan Pemadam ke database: $e");
      if (mounted) { // Pastikan widget masih ada di tree sebelum menampilkan SnackBar
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Gagal mengirim laporan Pemadam: ${e.toString()}", style: GoogleFonts.poppins(color: Colors.white)),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
                  ...
                  child: DropdownButtonFormField<String>(
                    ...
                    onChanged: (String? newValue) {
                      setState(() {
                        ...
                        // Logika untuk mengisi instansi terdekat secara otomatis jika diperlukan
                        // instansiController.text = _instansiPemadamMapping[newValue]!;
                      });
                    },
                    items: _daerahPemadamList.map<DropdownMenuItem<String>>((String value) {
            ...
            // --- Field Spesifik Pemadam ---
            Text("Jenis Kebakaran", style: GoogleFonts.poppins(color: primaryColor, fontWeight: FontWeight.bold, fontSize: 16)), // Menggunakan primaryColor
            const SizedBox(height: 8),
            TextFormField(
              controller: jenisKebakaranController,
              decoration: InputDecoration( // Ubah ke InputDecoration karena focusedBorder bukan const
                hintText: 'Misal: Kebakaran Bangunan/Lahan/Kendaraan',
                border: border,
                enabledBorder: border,
                focusedBorder: focusedBorder, // Menggunakan focusedBorder yang baru
                contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 14),
            ...
            Text("Korban Jiwa (Jika ada)", style: GoogleFonts.poppins(color: primaryColor, fontWeight: FontWeight.bold, fontSize: 16)), // Menggunakan primaryColor
            const SizedBox(height: 8),
            TextFormField(
              controller: korbanJiwaController,
              keyboardType: TextInputType.number,
              decoration: InputDecoration( // Ubah ke InputDecoration karena focusedBorder bukan const
                hintText: 'Jumlah korban jiwa',
                border: border,
                enabledBorder: border,
                focusedBorder: focusedBorder, // Menggunakan focusedBorder yang baru
                contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 14),
            ...
            Text("Estimasi Kerugian", style: GoogleFonts.poppins(color: primaryColor, fontWeight: FontWeight.bold, fontSize: 16)), // Menggunakan primaryColor
            const SizedBox(height: 8),
            TextFormField(
              controller: kerugianEstimasiController,
              decoration: InputDecoration( // Ubah ke InputDecoration karena focusedBorder bukan const
                hintText: 'Estimasi kerugian (contoh: Rp 50 Juta)',
                border: border,
                enabledBorder: border,
                focusedBorder: focusedBorder, // Menggunakan focusedBorder yang baru
                contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 14),
            ...
            TextFormField(
              ...
              maxLines: 4,
              decoration: InputDecoration( // Ubah ke InputDecoration karena focusedBorder bukan const
                hintText: 'Isi detail laporan kebakaran',
                border: border,
                enabledBorder: border,
                focusedBorder: focusedBorder, // Menggunakan focusedBorder yang baru
